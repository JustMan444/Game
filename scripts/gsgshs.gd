...
func play_special_songr(audio_path: String) -> void:
	is_final_credits = true
	print("МУЗЫКАЛЬНЫЙ МЕНЕДЖЕР: Запрос на специальный финал...")
	
	# 1. Загружаем твой новый финальный трек
	var new_track = load(audio_path)
	if not new_track:
		print("МУЗЫКАЛЬНЫЙ МЕНЕДЖЕР: Ошибка! Не нашли песню по пути: ", audio_path)
		return
		
	# 2. ИСПРАВЛЕНО: Работаем напрямую с твоей переменной music_player!
	if music_player:
		music_player.stop() # Намертво глушим Horizon или Bridge!
		music_player.stream = new_track
		music_player.volume_db = 0.0 # Выставляем громкость на полную
# ЗАМЕНИ СТРОЧКУ 120 НА ЭТУ:
		music_player.call_deferred("play")

		print("МУЗЫКАЛЬНЫЙ МЕНЕДЖЕР: Сейчас играет (ФИНАЛ): ", audio_path)
	else:
		# СТРАХОВКА: Если плеер еще не успел создаться в _ready
		music_player = AudioStreamPlayer.new()
		add_child(music_player)
		music_player.stream = new_track
		music_player.call_deferred("play")
		print("МУЗЫКАЛЬНЫЙ МЕНЕДЖЕР: Плеер создан с нуля для финала.")
